...
const SelectField = ({value, name, label, disabled, required, onChange, options}) => (
  <div className="form-group row">
    {label ? (<label htmlFor={`input_${name}`} className="col-sm-2 col-form-label">{label}</label>) : null}
    <div className="col-sm-10">
      <select className="form-control" id={`input_${name}`} name={name} disabled={disabled} value={value} onChange={onChange}>
